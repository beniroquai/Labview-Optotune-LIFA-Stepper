  ...
  if(command[0] == 0xFF && checksum_Test(command) == 0)
  {
    ...
    *********************************************************************************/
    case 0x34:  // IR Transmit  
      ...
      break;
    case 0xFC:
      writeDigitalPortMega(command);
      Serial.write('0');
	  break;
	case 0xFD:
	  writeDigitalByte(command);
	  Serial.write('0');
	  break;
	/*********************************************************************************
    ** Unknown Packet
...
void writeDigitalPortMega(unsigned char command[])
{
  int i = 0;
  for( i; i < 6; i++ )
  {
    if( i != 0 )
	{
	  digitalWrite(i * 8 + 0, (command[i+2] & 0x01) );
      digitalWrite(i * 8 + 1, (( command[i+2] >> 1) & 0x01) );
	}
    digitalWrite(i * 8 + 2, (( command[i+2] >> 2) & 0x01) );
    digitalWrite(i * 8 + 3, (( command[i+2] >> 3) & 0x01) );
    digitalWrite(i * 8 + 4, (( command[i+2] >> 4) & 0x01) );
    digitalWrite(i * 8 + 5, (( command[i+2] >> 5) & 0x01) );
    digitalWrite(i * 8 + 6, (( command[i+2] >> 6) & 0x01) );
    digitalWrite(i * 8 + 7, (( command[i+2] >> 7) & 0x01) );
  }
...
void writeDigitalByte(unsigned char command[])
{
	unsigned char pinStart = command[2];
	digitalWrite(pinStart + 0, (command[3] & 0x01) );
    digitalWrite(pinStart + 1, (( command[3] >> 1) & 0x01) );
    digitalWrite(pinStart + 2, (( command[3] >> 2) & 0x01) );
    digitalWrite(pinStart + 3, (( command[3] >> 3) & 0x01) );
    digitalWrite(pinStart + 4, (( command[3] >> 4) & 0x01) );
    digitalWrite(pinStart + 5, (( command[3] >> 5) & 0x01) );
    digitalWrite(pinStart + 6, (( command[3] >> 6) & 0x01) );
    digitalWrite(pinStart + 7, (( command[3] >> 7) & 0x01) );
}
